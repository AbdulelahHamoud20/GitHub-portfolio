...
int bstRecSize(struct bst_node* node){
  if (node != NULL){
    return(1+bstRecSize(node->left)+ bstRecSize(node->right));
  }
  else {
    return 0;
  }
...
//Helper functrion 
int getHeight(struct bst_node* node){

  if (node == NULL) return -1;

	int bst_left = getHeight(node->left);
	int bst_right = getHeight(node->right);
...
//helper function 
int getPathSum (struct bst_node* node, int sum){
...
int bst_path_sum(struct bst* bst, int sum) {
  if (bst->root == NULL) return 0;
	return getPathSum(bst->root, sum);
}
...
//Helper function for the range 

int getRangeSum(struct bst_node* node, int lower, int upper){
  if (node == NULL) return 0;
	...
	if (node) {
		...
		sum += getRangeSum(node->left, lower, upper);
		sum += getRangeSum(node->right, lower, upper);
	}
...
int bst_range_sum(struct bst* bst, int lower, int upper) {
  if (bst->root == NULL) return 0;
	return getRangeSum(bst->root, lower, upper);
}
