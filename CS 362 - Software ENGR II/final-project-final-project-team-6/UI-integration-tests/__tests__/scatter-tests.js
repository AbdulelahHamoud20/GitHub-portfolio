...
test("Correctly adds values in the scatter chart builder", async() => {
    initDomFromFiles(
...
describe("Alert tests for scatter chart builder", () => {
    // clear local storage
    beforeEach(() => {
...
test("All fields clear/reset correctly for scatter chart builder", async() => {
    initDomFromFiles(
        __dirname + "/../../src/scatter/scatter.html",
        __dirname + "/../../src/scatter/scatter.js"
    );
    ...
    await user.type(chartTitle, "My Scatter Plot");
    await user.type(axisLabelX, "X-Axis");
    await user.type(axisLabelY, "Y-Axis");
